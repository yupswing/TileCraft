  ...
  public var z2:Int;
	...
	public function getSlice(x:Int, gw:Int, gh:Int, dest:Slice, palette:Array<Int>, model:Model){
      ...
      {}
		case CYLINDER_UP:
  		{
  			...
  			var y:Int = Std.int((dest.y1 + dest.y2)/2); //toround
		...
		case CYLINDER_SIDE:
  		{
  			dt = (dt - 0.5)*2.0;
  			dt = Math.sqrt(1.0 - dt*dt)*0.999;
  			var z:Int = Std.int((dest.z1 + dest.z2)/2); //toround
		...
		case RAMP_UP:
  		{
  			dest.z2 = Std.int(Math.min(dest.z2, dest.z1 + Std.int(gh*dt*(z2 - z1))+1)); //toround
  			dest.normal = 0.9;
  		}
		case RAMP_DOWN:
  		{
  			dest.z2 = Std.int(Math.min(dest.z2, dest.z1 + Std.int(gh*(1.0 - dt)*(z2 - z1))+1)); //toround
  			dest.normal = 0.8;
  		}
		case ARCH_UP:
  		{
